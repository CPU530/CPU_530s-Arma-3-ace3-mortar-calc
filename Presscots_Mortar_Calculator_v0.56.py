...
"""IMPORTANT: V0.556 WILL SERVE AS A PREMISE FOR A REWRITE OF EXISTING CODE TO BETTER STRUCTURE THE APP TO BE USED IN A GUI; FOR ANY FOLLOWING THIS PROJECT IM AM STILL HERE ITS NOT DEAD BUT I AM BUSY WITH LIFE

CURRENTLY THE CODE GOES
WHERE ARE YOU
WHAT DO YOU WANT TO DO 
NOW PROVIDE INFO 
the main thing is that all of this is done in a singular main with very few sub functions 

i intend to break most of the "main" apart and have far more call functions that cna be linked to call buttons in a gui.
...
def gridmarks(charge0, charge1, charge2, global_mortar_pos):
    ...
    print("=" * 90)
    #since pos mortar and global mortar pos are dupes there is no reason for pos mortar to exist outside of my lazyness
    # this is a flawed check as it appears far after the mortar is already placed need to move to initial entry 
...
def main():
    ...
    #error checking for global mortar pos
    while True:
        if len(global_mortar_pos) % 2 == 1: 
        
            print("=" * 90)
            print("ERROR: The gridmark you entered is odd")
            global_mortar_pos = str(input("Enter the Grid position of your gun team: "))
            continue
        if not global_mortar_pos.isnumeric(): 
            
            print("=" * 90)
            print("ERROR: Your entry contains non-numeric characters.")
            global_mortar_pos = str(input("Enter the Grid position of your gun team: "))
            continue
        
        if global_mortar_pos.strip() == "":
            print("=" * 90)
            print("ERROR: Your input cannot be blank")
            global_mortar_pos = str(input("Enter the Grid position of your gun team: "))
            continue
        break
...
# this entire system will be depreciated when i institute a GUI as the logic checks will become a button press; however it is importat to keep the for now as it contains the call function that will be assiged to the buttons
