...
def main():
    ...
    humidity = 0  #(int (input("Enter the humidity value as shown on the kestrel: ")))
    temperature = 0  #int (input("Enter the temperature value as shown on the kestrel: "))
    pressure = 0  #int (input("Enter the Air Pressure value as shown on the kestrel: "))
    hWindSpeed = 0  #float (input("Enter the Headwind value as shown on the kestrel: "))
    cWindSpeed = 0  #float (input("Enter the Crosswind value as shown on the kestrel: "))
    ''' positive wind speed denotes the travel from left to right '''
    print('=' * 90)
    print('OUTPUT')
    ...
    '''Enviremental calculations'''

    air_density = ((pressure * 100 - humidity * 610.78 * 10**(
        (7.5 * temperature) /
        (temperature + 237.3))) * 0.028964 + humidity * 610.78 * 10**(
            (7.5 * temperature) /
            (temperature + 237.3)) * 0.018016) / (8.314 *
                                                  (temperature + 273.15))

    #crosswind = math.sin(math.radians(angle)) * cWindSpeed
    #headwind = math.cos(math.radians(angle)) * hWindSpeed
    ''' and alternate way to get air density '''
    sat = 610.78 * 10**((7.5 * temperature) / (temperature + 237.3))
    vap = humidity * sat
    pp = pressure - vap
    ad = (pp * 0.028964 + vap * 0.018016) / (8.314 * (temperature + 273.15))
    ''' distance CALC'''
    ...
    #NW
    if dx < 0 and dy > 0:
        azimuth = 6400 + 17.77777777777777777778 * math.degrees(
            math.atan(dx / dy))
        print('NW')
    #SE
    if dx > 0 and dy < 0:
        azimuth = 3200 + 17.77777777777777777778 * math.degrees(
            math.atan(dx / dy))
        print('SE')
    #SW
    if dx < 0 and dy < 0:
    ...
    '''determine why float division by zero'''
    '''Azimuth Variables'''
    if type(range0Upper) and type(range0Lower) == int:
        crossWindAdjC0Near = cWindSpeed * charge0[range0Lower][5]
        crossWindAdjC0Far = cWindSpeed * charge0[range0Upper][5]

    if type(range1Upper) and type(range1Lower) == int:
        crossWindAdjC1Near = cWindSpeed * charge1[range1Lower][5]
        crossWindAdjC1Far = cWindSpeed * charge1[range1Upper][5]

    if type(range2Upper) and type(range2Lower) == int:
        crossWindAdjC2Near = cWindSpeed * charge2[range2Lower][5]
        crossWindAdjC2Far = cWindSpeed * charge2[range2Upper][5]

    if charge0Truth == 1:
        nearAzimuth0ADJ = azimuth - crossWindAdjC0Near
        farAzimuth0ADJ = azimuth - crossWindAdjC0Far
        avgADJAzimuth0 = ((nearAzimuth0ADJ / farAzimuth0ADJ) / 2)

    if charge1Truth == 1:
        nearAzimuth1ADJ = azimuth - crossWindAdjC1Near
        farAzimuth1ADJ = azimuth - crossWindAdjC1Far
        avgADJAzimuth1 = ((nearAzimuth1ADJ / farAzimuth1ADJ) / 2)

    if charge2Truth == 1:
        nearAzimuth2ADJ = azimuth - crossWindAdjC2Near
        farAzimuth2ADJ = azimuth - crossWindAdjC2Far
        avgADJAzimuth2 = ((nearAzimuth2ADJ / farAzimuth2ADJ) / 2)
    '''Elavation Variables '''
    #  hWindSpeed
    ...
    '''decline collum[10]'''
    if air_density < 0:
        air_density * charge[elevation1][10]
        air_density * charge[elevation2][10]
    '''incline collum[11]'''
    if air_density > 0:
        air_density * charge[elevation1][11]
        air_density * charge[elevation2][11]

    adjustmentA = 0  #crosswind*charge []
    adjustmentE = 0  #headwind*charge

    adjAzimuth = 1  #azimuth + adjustmentA
    adjElevation1 = 1  #elevation1 + adjustmentE
    adjElevation2 = 1  #elevation2 + adjustmentE
    adjElevationAVG = ((adjElevation2 + adjElevation1) / 2)
    ''' The print statments '''
    print('=' * 90)
    print('=' * 90)
    ...
    if charge0Truth == 1:
        print(
            f' [ Charge 0 Near Elevation:{nearEL0}, Far Elevation:{farEL0}] Average Elevation for Charge 0 {avgelevation0}'
        )
    else:
        print(' Charge 0 is invalid')
    if charge1Truth == 1:
        print(
            f' [ Charge 1 Near Elevation:{nearEL1}, Far Elevation:{farEL1}] Average Elevation for Charge 1 {avgelevation1}'
        )
    else:
    ...
    if charge2Truth == 1:
        print(
            f' [ Charge 2 Near Elevation:{nearEL2}, Far Elevation:{farEL2}] Average Elevation for Charge 2 {avgelevation2}'
        )
    else:
    ...
    '''
    print('='*90)
     #   print (f' ADJ:FIRE MISSION \n Azimuth : {adjAzimuth} \n Elevation range {adjElevation1}-{adjElevation2} \n Elevation Average {adjElevationAVG}\n Distance from Target {distance} Meters')
    print('='*90)
    print('='*90)
    '''
